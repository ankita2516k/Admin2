...
export default function UserManagement(){
  ...
  const filtered = (query && query.trim())
    ? allUsers.filter(u => u.name.toLowerCase().includes(query.toLowerCase()) || u.email.toLowerCase().includes(query.toLowerCase()))
    : allUsers;
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div className="page-actions">
          <input className="small-search" placeholder="Search users..." value={query} onChange={e=>setQuery(e.target.value)} />
      ...
      <div className="table-card">
        <table className="data-table">
          <thead><tr><th>Name</th><th>Email</th><th>Registered Events</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            {filtered.map(u => (
              <tr key={u.id}>
                <td>{u.name}</td>
                <td>{u.email}</td>
                <td>{u.events}</td>
                <td><span className={`badge ${u.status==="Active"?"green": u.status==="Pending"?"yellow":"red"}`}>{u.status}</span></td>
                <td>
